...
class AppStore extends Component {
  state = {
    ...
    filteredAppList: appsList.filter(
      item => item.category === tabsList[0].tabId,
    ),
  }
  ...
  onSearchInput = event => {
    ...
    const newList = appsList.filter(
      item =>
        item.appName.toLowerCase().includes(event.target.value.toLowerCase()) &&
        item.category.includes(activeTabId),
    )
    this.setState({
      searchInput: event.target.value,
      filteredAppList: newList,
    })
  }
  ...
  render() {
    const {searchInput, activeTabId, filteredAppList} = this.state
    return (
      <div className="bg-container1">
        <h1 className="main-heading1">App Store</h1>
        <div className="searchbar-container1">
          ...
          <img
            ...
            alt="search icon"
            className="search-icon-image1"
          />
        </div>
        <ul className="tabItem-container1">
          {tabsList.map(item => (
            <TabItem
              key={item.tabId}
              tabId={item.tabId}
              displayText={item.displayText}
              onNewTabSelection={this.onNewTabSelection}
              activeTab={activeTabId}
            />
          ))}
        </ul>
        <ul className="appItem-container1">
          {filteredAppList.map(item => (
            <AppItem appName={item.appName} imageUrl={item.imageUrl} />
          ))}
        </ul>
      </div>
    )
